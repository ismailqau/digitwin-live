...
router.post('/documents', authMiddleware, requirePermission('knowledge:write'), uploadDocument);
```
...
router.get('/analytics', authMiddleware, requireAnyRole(['admin', 'analyst']), getAnalytics);
```
...
router.post('/voice/create', authMiddleware, requireSubscriptionTier('pro'), createVoiceModel);
```
...
```typescript
const freePermissions = ['conversation:create', 'conversation:read', 'knowledge:read'];
```
...
```typescript
const proPermissions = [...freePermissions, 'knowledge:write', 'voice:create', 'face:create'];
```
...
```typescript
const enterprisePermissions = [...proPermissions, 'analytics:read', 'team:manage'];
```
...
// Usage
router.delete('/documents/:id', authMiddleware, requireOwnership('document'), deleteDocument);
```
...
```typescript
router.post('/new-feature', authMiddleware, requirePermission('new:permission'), newFeatureHandler);
```
...
```typescript
router.get('/special-access', authMiddleware, requireRole('new-role'), specialHandler);
```
